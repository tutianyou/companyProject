...
public class SearchNingDeTotalDataExportExcelController {

	
		@Autowired
		private SearchTotalDataExportExcelService searchNingDeTotalDataExportExcelService;
		
		@RequestMapping("/searchNingDeTotalDataExportExcel")
		public void searchNingDeTotalDataExportExcel(){
	        ...
	        HSSFSheet sheet=wb.createSheet("宁德数据报表");
	        ...
	        row2.createCell(3).setCellValue("求和项:发票金额");
	        row2.createCell(4).setCellValue("求和项:进项票金额");
			...
			/**查询宁德的衍生数据*/
			String searchNingDeTotalDataExportExcel = searchNingDeTotalDataExportExcelService.searchNingDeTotalDataExportExcel();
			List<ExpandPojo> parseArray = JSON.parseArray(searchNingDeTotalDataExportExcel, ExpandPojo.class);
	        ...
	        response.reset();
	        response.setHeader("Content-disposition", "attachment; filename=NingDeData.xls");
