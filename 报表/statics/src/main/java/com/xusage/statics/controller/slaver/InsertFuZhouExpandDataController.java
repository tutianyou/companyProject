...
import com.xusage.statics.service.slaver.InsertFuZhouExpandDataService;
...
public class InsertFuZhouExpandDataController {

	@Autowired
	private InsertFuZhouExpandDataService insertFuZhouExpandDataService;
	...
	@RequestMapping("/insertFuZhouExpandData")
	...
	for (CommentPojo commentPojo : searchFuZhouProvinceFinanical) {
		String selectFuZhouSubjectCode = selectSubjectCodeService.selectFuZhouSubjectCode();
		int catalogNumber = 0;
		Map<String, Object> map = JSON.parseObject(selectFuZhouSubjectCode);
		for (Map.Entry<String, Object> entry : map.entrySet()) {
			   ...
			   catalogNumber+=1;
			   String str = entry.getKey().toString().split("_")[1];
			   if(entry.getKey().toString().split("_")[1].equals("1122.10.01.002.001")) {
			   ...
			   else if(entry.getKey().toString().split("_")[1].equals("2202.10.01.002.001")) {
	    ...
	    String expandContent  = jacksonObjectMapper.writeValueAsString(expand);
	  	...
	  	Integer expandTotalData = insertFuZhouExpandDataService.insertFuZhouExpandData("福州衍生数据", expandContent);
